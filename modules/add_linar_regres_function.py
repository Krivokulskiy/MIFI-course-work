...
def compare_models_linar_regression(
	...
	# Для ROC, accuracy, recall переведём задачу в бинарную: выше/ниже медианы
	...
	models = {
		...
		'MLPRegressor': MLPRegressor(hidden_layer_sizes=(100, 50), max_iter=1000, random_state=random_state),
		'SVR': SVR()
	}
	...
	param_grids = {
		...
		'MLPRegressor': {'hidden_layer_sizes': [(100,), (100, 50)], 'max_iter': [500, 1000]},
	...
	for key, reducer in reducers.items():
		...
		y_bin_train_dict[key] = y_bin_train_red
		y_bin_test_dict[key] = y_bin_test_red
	...
	for key in reducers:
		for name, model in models.items():
			...
			mse = mean_squared_error(y_test_dict[key], y_pred)
			r2 = r2_score(y_test_dict[key], y_pred)
			acc = accuracy_score(y_bin_test_dict[key], y_pred_bin)
			recall = recall_score(y_bin_test_dict[key], y_pred_bin)
			try:
				roc_auc = roc_auc_score(y_bin_test_dict[key], y_pred)
			except:
				roc_auc = np.nan
			...
			# ROC-кривая
			fpr, tpr, _ = roc_curve(y_bin_test_dict[key], y_pred)
			roc_auc_val = auc(fpr, tpr)
			roc_curves_all[key][name] = (fpr, tpr, roc_auc_val)

	# --- Вывод лучших параметров и метрик для каждой модели ---
	...
	for mode in mode_names:
		...
		for model in model_names:
			...
			print(f"{model}:")
			print(f"  Лучшие параметры: {params if params else 'default'}")
			print(f"  Accuracy: {res['accuracy']:.3f}, Recall: {res['recall']:.3f}, ROC_AUC: {res['roc_auc']:.3f}, R2: {res['r2']:.3f}")

	# --- Сравнительные горизонтальные столбчатые графики (только значения метрик) ---
	...
	for i, metric in enumerate(metrics):
		...
		for j, mode in enumerate(mode_names):
			...
			bars = axes[i].barh(
				y_pos + (j-1)*bar_height, values, height=bar_height, color=['royalblue', 'orange', 'green'][j], label=mode, alpha=0.85, edgecolor='black'
			)
		...
		if i == 0:
			handles = [mpatches.Patch(color=['royalblue', 'orange', 'green'][k], label=mode_names[k]) for k in range(len(mode_names))]
			axes[i].legend(handles=handles, loc='lower center', bbox_to_anchor=(0.5, 1.05), ncol=3, frameon=False, fontsize=12)

	plt.tight_layout()
	plt.show()

	# --- Сравнительные ROC-кривые ---
